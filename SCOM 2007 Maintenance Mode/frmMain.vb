...
Public Class frmMain
	...
	Private Structure pDefaultSettings
		...
		Shared sS_Excluded As String = vbNullString
	End Structure
	...
	Private Sub frmMain_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
		...
		Me.Text = "SCOM 2007 Maintenance Mode - v" & Application.ProductVersion
		...
		' Build maintenance results listview control
		With lstMaint_Results
			.Clear()
			With .Columns
				.Add("name", "", lstMaint_Results.Width - 80 - SystemInformation.VerticalScrollBarWidth - 4)
				.Add("a", "", 20)
				.Add("b", "", 20)
				.Add("c", "", 20)
				.Add("d", "", 20)
		...
		' Build search results listview control
...
#Region "TAB: (tpL) Server List Entry"
		...
		Try
			...
			Using sw As StreamWriter = New StreamWriter(sFilename)
				...
				If (dResult = Windows.Forms.DialogResult.Yes) Then
					...
					'sw.WriteLine("#cluster   = " & chkEntry_CheckForClusters.Checked.ToString)
	...
	Private Sub txtEntry_ServerList_LostFocus(sender As Object, e As System.EventArgs) Handles txtEntry_ServerList.LostFocus
		...
		' Remove "Excluded" servers first...
		If (pSettings.sS_Excluded <> vbNullString) Then
			For Each sDis As String In pSettings.sS_Excluded.Split(","c)
	...
	Private Sub cmdEntry_Verify_Click(sender As System.Object, e As System.EventArgs) Handles cmdEntry_Verify.Click
		...
		If (cServer.Count = 0) Then
			...
			Dim comClass As MonitoringClassCriteria = New MonitoringClassCriteria("Name='Microsoft.Windows.Computer'")
			Dim comClasses As ReadOnlyCollection(Of MonitoringClass) = mg.GetMonitoringClasses(comClass)
			Dim comMO As ReadOnlyCollection(Of MonitoringObject) = mg.GetMonitoringObjects(comClasses(0))

			Dim lItem As ListViewItem
			For Each sServer As MonitoringObject In comMO
				' Check if server is part of the "Excluded" list...
				Dim bExcluded As Boolean = False
				If (pSettings.sS_Excluded <> vbNullString) Then
					If (InStr(pSettings.sS_Excluded.ToLower, sServer.Name.ToLower.Trim, CompareMethod.Text) > 0) Then bExcluded = True
				End If
...
#Region "TAB: (tpR) Maintenance Results"
	...
	Private Sub doScan(ByVal bStart_MM As Boolean)
		...
		Dim comClass As MonitoringClassCriteria = New MonitoringClassCriteria("Name='Microsoft.Windows.Computer'")
		Dim hsvClass As MonitoringClassCriteria = New MonitoringClassCriteria("Name='Microsoft.SystemCenter.HealthService'")
		Dim hswClass As MonitoringClassCriteria = New MonitoringClassCriteria("Name='Microsoft.SystemCenter.HealthServiceWatcher'")
		Dim cluClass As MonitoringClassCriteria = New MonitoringClassCriteria("Name='Microsoft.Windows.Cluster'")
		Dim comClasses As ReadOnlyCollection(Of MonitoringClass) = mg.GetMonitoringClasses(comClass)
		Dim hsvClasses As ReadOnlyCollection(Of MonitoringClass) = mg.GetMonitoringClasses(hsvClass)
		Dim hswClasses As ReadOnlyCollection(Of MonitoringClass) = mg.GetMonitoringClasses(hswClass)
		Dim cluClasses As ReadOnlyCollection(Of MonitoringClass) = mg.GetMonitoringClasses(cluClass)
			...
			For Each lItem As ListViewItem In gGroup.Items
				...
				If (lItem.Text.StartsWith("!x-") = False) Then
					Try
						...
						Dim comMOC As MonitoringObjectCriteria = New MonitoringObjectCriteria("DNSName='" & lItem.Text & "'", comClasses(0))
						Dim hswMOC As MonitoringObjectCriteria = New MonitoringObjectCriteria("HealthServiceName='" & lItem.Text & "'", hswClasses(0))
						Call aLog("         Getting monitoring object - Computer")
						Dim comMO As ReadOnlyCollection(Of MonitoringObject) = mg.GetMonitoringObjects(comMOC)
						Dim hsvMO As ReadOnlyCollection(Of MonitoringObject) = Nothing
						Dim hswMO As ReadOnlyCollection(Of MonitoringObject) = Nothing
						Dim cluMO As ReadOnlyCollection(Of MonitoringObject) = Nothing
						Dim cluMOL As List(Of MonitoringObject) = New List(Of MonitoringObject)()

						Dim bIsCluster As Boolean = False
						If (comMO.Count > 0) Then
							Call aLog("         Checking for cluster resources")
							Dim sClusterName As String = vbNullString
							Dim clItems As List(Of MonitoringObject) = Nothing
							cluMO = comMO(0).GetRelatedMonitoringObjects(Common.TraversalDepth.Recursive)

							For Each mo As MonitoringObject In cluMO
								...
								If ((mo.DisplayName IsNot Nothing) AndAlso (mo.DisplayName = "Cluster Service")) Then
									bIsCluster = True
									Call aLog("            Cluster resources found")
								End If
							Next

							If (bIsCluster = True) Then
								Dim cluMOC As MonitoringObjectCriteria = New MonitoringObjectCriteria("Name='" & sClusterName & "'", cluClasses(0))
								cluMOL.AddRange(mg.GetMonitoringObjects(cluMOC))
							End If

							Try
								Call aLog("         Getting related monitoring object - Health Service")
								hsvMO = comMO(0).GetRelatedMonitoringObjects(hsvClasses(0))
							Catch ex As Exception
								Call aLog("            Failed, but continuing")
							End Try

							Try
								Call aLog("         Getting related monitoring object - Health Service Watcher")
								hswMO = mg.GetMonitoringObjects(hswMOC)
							Catch
								Call aLog("            Failed, but continuing")
							End Try
						Else
							...
							lItem.SubItems(3).Text = "ICON:5"
							lItem.SubItems(4).Text = "ICON:5"
							Exit Try
						End If
						...
						If (bStart_MM = True) Then
							...
							If (comMO(0).InMaintenanceMode = True) Then sMode = "update" Else sMode = "start"
						Else
							'STOP MAINTENANCE MODE (but only if actually in maintenance mode)...
							If (comMO(0).InMaintenanceMode = True) Then sMode = "stop" Else sMode = vbNullString
						End If
						...
						If (sMode <> vbNullString) Then
							lItem.SubItems(1).Text = "ICON:" & CInt(setMonitoring(comMO, sMode, "Computer"))
							If (bIsCluster = True) Then
								lItem.SubItems(2).Text = "ICON:" & CInt(setMonitoring(cluMOL.AsReadOnly, sMode, "Cluster"))
							Else
								If (lItem.SubItems(2).Text = vbNullString) Then lItem.SubItems(2).Text = "ICON:5"
							End If
							lItem.SubItems(3).Text = "ICON:" & CInt(setMonitoring(hsvMO, sMode, "Health Service"))
							lItem.SubItems(4).Text = "ICON:" & CInt(setMonitoring(hswMO, sMode, "Health Service Watcher"))
							Call aLog("      Done")
						Else
					...
					Catch ex As Exception
						...
						lItem.SubItems(3).Text = "ICON:5"
						lItem.SubItems(4).Text = "ICON:5"
						Call aLog("***      ERROR During Maintenance Configuration" & vbCrLf & "***      " & ex.Message)
					End Try
				Else
	...
	''' <remarks> </remarks>  
	Private Function setMonitoring(ByVal mo As ReadOnlyCollection(Of MonitoringObject), ByRef start_update_stop As String, ByRef sOpTarget As String) As MonitoringReturnStatus
		...
		If ((mo IsNot Nothing) AndAlso (mo.Count > 0)) Then
			Try
				Application.DoEvents()
				...
				Else
					...
					Select Case start_update_stop.ToLower
						Case "start" : mo(0).ScheduleMaintenanceMode(Now.ToUniversalTime, dtpEntry_Window.Value.ToUniversalTime, iMMR, txtEntry_Comment.Text, Common.TraversalDepth.Recursive)
						Case "update" : mo(0).UpdateMaintenanceMode(dtpEntry_Window.Value.ToUniversalTime, iMMR, txtEntry_Comment.Text, Common.TraversalDepth.Recursive)
						Case "stop" : mo(0).StopMaintenanceMode(Now.ToUniversalTime, Common.TraversalDepth.Recursive)
					End Select
				End If
	...
	Private Sub buildResultsTable()
		...
		For Each sServer As String In dServers
			If (sServer.Length > 1) Then
				...
				lItem.SubItems.Add(vbNullString)									' Health Service
				lItem.SubItems.Add(vbNullString)									' Health Watcher Service
				lItem.SubItems.Add(Mid(sServer, InStr(sServer, ".") + 1).ToLower)	' HIDDEN (Domain Name)
				If (lItem.SubItems(5).Text.ToLower = sServer.ToLower) Then lItem.SubItems(5).Text = " No Domain"
	...
	Private Sub lnkMaint_ColumnDesc_LinkClicked(sender As System.Object, e As System.Windows.Forms.LinkLabelLinkClickedEventArgs) Handles lnkMaint_ColumnDesc.LinkClicked
		Dim sMsg As String = vbNullString
		sMsg = sMsg & "Column Header Descriptions" & vbCrLf & "Please check the help for more information" & vbCrLf & vbCrLf
		sMsg = sMsg & "|  Server Name                |  1  |  2  |  3  |  4  |" & vbCrLf & vbCrLf
		sMsg = sMsg & "          1. Computer Resource" & vbCrLf
		sMsg = sMsg & "              2. Cluster Resource" & vbCrLf
		sMsg = sMsg & "                  3. Health Service" & vbCrLf
		sMsg = sMsg & "                      4. Health Service Watcher"
...
#Region "TAB: (tpA) Admin Options"
	...
	Private Sub cmdAdmin_Save_Click(sender As System.Object, e As System.EventArgs) Handles cmdAdmin_Save.Click
		...
		Call WritePrivateProfileString("Settings", "Excluded", txtAdmin_DefaultsExcluded.Text, sSettingsPath)
	...
	Private Sub loadOptions()
		...
		Else
			...
			pSettings.sS_Excluded = readSetting("Excluded", "Settings", vbNullString)
	...
	Private Sub cmdAdmin_Reset_Click(sender As System.Object, e As System.EventArgs) Handles cmdAdmin_Reset.Click
		...
		txtAdmin_DefaultsExcluded.Text = vbNullString
	End Sub
		...
		If (sServerName.StartsWith("#")) Then
			...
			If (sValue <> vbNullString) Then
				Select Case True
					...
					Case sName.ToLower.StartsWith("#cluster")
						'If ((sValue.ToLower = "true") Or (sValue.ToLower = "false")) Then chkEntry_CheckForClusters.Checked = CBool(sValue)
